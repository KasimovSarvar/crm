...
from .serializers import (StudentSerializer, PaymentSerializer,
                          ...
                          PaymentCreateSerializer, CommentSerializer
)
...
def lead_update_view(request, pk):
    ...
    if lead.admin != request.user:
        return Response({'message': 'You can only update your own leads'}, status=status.HTTP_403_FORBIDDEN)
    if lead.status == 'canceled':
        return Response({'message': 'You con not update ->  Canceled lead..!'}, status=status.HTTP_400_BAD_REQUEST)
    if is_student:
        return Response({'message': 'if this lead already has student -> you can not update'},
                        status=status.HTTP_400_BAD_REQUEST)
